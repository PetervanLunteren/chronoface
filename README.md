...
- 🧭 **Time-aware scanning** – Reads EXIF `DateTimeOriginal`, skips invalid files, and groups photos
  into day, week, month, or year buckets.
- 😀 **CPU face pipeline** – Uses YuNet for detection and SFace for embeddings with ONNX Runtime on
  the CPU, no GPU required.
- 🧹 **Cache & resume** – Hash-based cache of thumbnails and embeddings avoids recomputation when you
  rescan the same folder.
- 🧑‍💻 **Human-in-the-loop review** – Accept or reject faces, merge/split clusters, and track skipped
  images with clear reasons.
- 🖼️ **Collage exporter** – Render grid collages per bucket with configurable layout, background,
  tile size, and filters.
- 🔌 **Local-only** – No telemetry or network calls during processing. Works on Windows, macOS, and
...
├── scripts/            # Helper scripts (dev runner)
└── backend/data/       # Sample photos with EXIF metadata
```
...
1. **Scan** – Walk the provided folder recursively, reading EXIF `DateTimeOriginal`. Photos without a
   valid timestamp are skipped and reported.
2. **Detect** – Downscale images if requested, run YuNet on the CPU, filter by minimum face size, and
   emit thumbnails + embeddings.
3. **Cluster** – Group embeddings per bucket with DBSCAN (cosine distance), assign noise to `noise`.
4. **Review** – UI lets you accept/reject faces, merge clusters, or split faces into a new cluster.
5. **Collage** – Select faces (accepted only, or include unreviewed) and render a deterministic grid;
